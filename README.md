...
5) (Рекомендуется) Загрузите полные демо-данные: 10 пользователей, 15 автомобилей, историю ТО/ремонта и 20 заявок с разными статусами:
...
7) (Опционально, если не загружаете fixture) Засидьте минимальный демо-набор через management command:
...
- `client`: создание заявки + просмотр своих заявок
...
### Демо-доступы (после `loaddata fixtures/demo.json`)
...
- `fleet_manager` / `demo12345`

- `dispatcher` / `demo12345`

- `employee` / `demo12345`

- `engineer` / `demo12345`

- `sales_employee` / `demo12345`

- `client` / `demo12345`

- `client_acme` / `demo12345`

- `client_beta` / `demo12345`

### Расчётные показатели

Расчёты доступны сервисному администратору в разделе `/reports/admin/`.

В сводке отчётов рассчитываются:

- загрузка автопарка в процентах;
- занятые машино-дни и свободные машино-дни;
- средняя длительность заявки;
- суммарный пробег по завершённым заявкам;
- среднесуточный пробег;
- количество, средняя длительность и максимум просрочек возврата;
- топ автомобилей по загрузке.

Основные формулы:

```text
дней использования = дата окончания - дата начала + 1
загрузка автопарка (%) = занятые машино-дни / доступные машино-дни * 100
доступные машино-дни = количество автомобилей * дней в расчётном периоде
пробег по заявке = пробег при возврате - пробег при выдаче
среднесуточный пробег = пробег по заявке / дней использования
дней до ТО = оставшийся пробег до ТО / среднесуточный пробег
```

В карточке конкретного отчёта `/reports/admin/<id>/` показываются расчёты по заявке: дни использования, просрочка возврата, пробег при выдаче и возврате, пробег по заявке, среднесуточный пробег и прогноз даты следующего ТО при текущем темпе эксплуатации.
